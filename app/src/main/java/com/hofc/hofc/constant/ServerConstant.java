package com.hofc.hofc.constant;

public class ServerConstant {
	public static String SERVER_URL_PREFIX = "https://";
	public static String SERVER_URL = "quiet-wave-7010.herokuapp.com";
    public static int SERVER_PORT = 0;
	public static String CALENDRIER_CONTEXT = "calendrier";
	public static String CLASSEMENT_CONTEXT = "classement";
	public static String ACTUS_CONTEXT = "actus";
    public static String NOTIF_CONTEXT = "registerPush";
    public static String PARSE_PAGE_CONTEXT = "parsePage";
    public static String AGENDA_CONTEXT = "agenda";
	
	public static final int NOMBRE_JOUR_SYNCHRO = 7;
}
